const sdk = require('node-appwrite');

const client = new sdk.Client();
const databases = new sdk.Databases(client);

client
    .setEndpoint('https://fra.cloud.appwrite.io/v1')
    .setProject('6971430c00318957e6d7')
    .setKey('standard_');

const DATABASE_ID = '6971454f002823a65775';
const COLLECTION_ID = 'admins';

async function updateAdmin() {
    console.log('🔐 Updating admin credentials...');

    const email = 'dodahassan@gmail.com';
    const password = 'dodahassan@gmail.com';

    try {
        // Check if admin already exists
        const existing = await databases.listDocuments(DATABASE_ID, COLLECTION_ID, [
            sdk.Query.equal('username', email)
        ]);

        if (existing.total > 0) {
            console.log('📝 Admin exists, updating password...');
            await databases.updateDocument(
                DATABASE_ID,
                COLLECTION_ID,
                existing.documents[0].$id,
                {
                    password: password,
                    updatedAt: new Date().toISOString()
                }
            );
            console.log('✅ Admin password updated!');
        } else {
            console.log('➕ Admin does not exist, creating new record...');
            await databases.createDocument(
                DATABASE_ID,
                COLLECTION_ID,
                sdk.ID.unique(),
                {
                    username: email,
                    password: password,
                    name: 'Doda Hassan',
                    role: 'admin',
                    createdAt: new Date().toISOString()
                },
                ['read("any")', 'update("any")', 'delete("any")']
            );
            console.log('✅ Admin "dodahassan@gmail.com" created successfully!');
        }
    } catch (error) {
        console.error('❌ Error updating admin:', error.message);
    }
}

updateAdmin();
